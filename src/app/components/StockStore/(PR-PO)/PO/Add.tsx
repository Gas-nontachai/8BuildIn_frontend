...
import { usePurchaseOrder, useUnit } from "@/hooks/hooks";
import { PurchaseOrder } from '@/misc/types';
import { decimalFix } from "@/utils/number-helper";

const { insertPurchaseOrder } = usePurchaseOrder();
...
const PurchaseOrderAdd: React.FC<Props> = ({ onRefresh, onClose, open }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await onClose();
            await insertPurchaseOrder(insertData);
            setPurchaseOrder({
                po_id: '',
                pr_id: '',
                supplier_id: '',
            ...
            <DialogContent>
                <Grid container spacing={2}>
                    <Grid size={12}>
                        ...
                        <textarea
                            value={purchaseOrder.po_note}
                            onChange={(e) =>
                                setPurchaseOrder({ ...purchaseOrder, po_note: e.target.value })
                            }
